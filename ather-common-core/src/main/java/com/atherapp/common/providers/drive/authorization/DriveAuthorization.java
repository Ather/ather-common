 ...
 *
 */
package com.atherapp.common.providers.drive.authorization;
 ...
 * @author Michael Haas
 */
public class DriveAuthorization extends
		ProviderAuthorization<DriveAuthorization, DriveConfiguration, DriveCredential, DriveFile, Drive, DriveService> {
	...
	private static void loadSecrets() {
		try {
			...
			clientSecrets = GoogleClientSecrets
					.load(JSON_FACTORY, new InputStreamReader(new FileInputStream(new File(AtherConfiguration.CONFIGURATION_LOCATION + "providers/drive/client_secret.json"))));
		} catch (IOException e) {
			DRIVE_AUTHORIZATION_LOGGER
					.error("Failed to initialize google client secrets for Google Drive authorization", e);
		}
	}
	...
	public String getAuthorizationUrl(boolean useSsl) {
		return new GoogleAuthorizationCodeRequestUrl(clientSecrets, getMatchingRedirectUri(useSsl), Arrays
				.asList(DriveScopes.DRIVE)).setState(provider.getUuid().toString()).setApprovalPrompt("force")
	...
	private String getMatchingRedirectUri(boolean useSsl) {
		...
		for (String redirectUri : clientSecrets.getDetails().getRedirectUris()) {
			if (redirectUri.contains("/settings/providers/callback/drive")) {
				...
				if (useSsl && redirectUri.startsWith("https://")) return returnUri;
			}
		}
	...
	public void setAuthorization(boolean useSsl, String responseToken) {
		try {
			...
			Credential tempCredential = new GoogleCredential.Builder()
					.setRefreshListeners(Collections.singleton(new DriveCredentialRefreshListener(this.provider)))
					.setTransport(transport).setJsonFactory(JSON_FACTORY).setClientSecrets(clientSecrets).build()
					.setFromTokenResponse(new GoogleAuthorizationCodeTokenRequest(transport, JSON_FACTORY, clientSecrets
							.getDetails().getClientId(), clientSecrets.getDetails()
							                                                              .getClientSecret(), responseToken, getMatchingRedirectUri(useSsl))
							                      .execute());

			com.google.api.services.drive.Drive tempDrive = new com.google.api.services.drive.Drive.Builder(transport, JSON_FACTORY, tempCredential)
					.setApplicationName(DriveService.APPLICATION_NAME).build();
	...
	public DriveService getProviderService() {
		if (service == null) service = new DriveService(this.provider);
	...
	public static class DriveCredentialRefreshListener implements CredentialRefreshListener {
		...
		public void onTokenResponse(Credential credential, TokenResponse tokenResponse) throws IOException {
			DRIVE_AUTHORIZATION_LOGGER
					.info("Persisting updated credential for provider: " + this.drive.getUuid().toString());
			this.drive.persistCredential(new DriveCredential(credential));
		}

		@Override
		public void onTokenErrorResponse(Credential credential, TokenErrorResponse tokenErrorResponse) throws
				IOException {
			DRIVE_AUTHORIZATION_LOGGER.error("Failed to refresh drive authorization token: " + drive.getUuid()
					.toString(), tokenErrorResponse.toString());
		}
	}
}
