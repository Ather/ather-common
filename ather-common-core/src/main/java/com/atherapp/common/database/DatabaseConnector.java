...
import com.atherapp.common.configuration.ConfigurationContents;
...
public class DatabaseConnector {
	...
	private synchronized static void generatePools() {
		AtherConfiguration.contents().getDatabaseProfiles().forEach((K, V) -> {
			...
			source.setUrl(String.format("jdbc:mysql://%s:%d/%s?verifyServerCertificate=false&useSSL=true", V
					.getDatabaseIp(), V.getDatabasePort(), V.getDatabaseSchema()));
	...
	public static Connection getConnection() throws SQLException {
		ConfigurationContents.DatabaseProfile defaultProfile = AtherConfiguration.contents().getDatabaseProfile();
		if (defaultProfile != null) return getConnection(defaultProfile.getConnectionAlias());
